...
# triangulærfordeling som brukes til å estimere hvor mange gjenutsatte laks som dør
# de valgte verdiene gir et gjennomsnitt på 7 % døde
andel_dode_gjenuts <- rtriangle(
  n_sim,
  1 - 0.96,
  1 - 0.85,
  1 - 0.93
...
#-----------------------------------------------------------------------------------------------------------------
# importer datafilene for alle vassdragene i elveliste slik at alle er tilgjengelige i ett datasett
#-----------------------------------------------------------------------------------------------------------------
inkl_kolonner <- c(
  "Vekt_MS_u3kg",
  "Vekt_MS_37kg",
...
for (m in indeks_simuler) {
  ...
  d <- d %>%
    # feil navn på Gjen_vekt_o7kg i input, ta vekk når dette er fikset
    rename(Gjen_vekt_o7kg = Gjen_vekto7kg) %>%
    # legg til region-id i datasett
    mutate(Region = elveliste$RegionNr[m])

  # sjekk om filen inneholder fangst fra hovedstreng (MS), legg til med nuller dersom de ikke finnes
  for (kol_navn in inkl_kolonner) {
    if (!(kol_navn %in% names(d))) {
      d[[kol_navn]] <- 0
    }
  }

  if (m == indeks_simuler[1]) {
    elvedata <- d
  } else {
...
# liste over regioner i elvedataene
indeks_regioner <- unique(elvedata$Region)
# liste over år i elvedataene
indeks_aar <- unique(elvedata$Aar)

for (r in indeks_regioner) {
  for (a in indeks_aar) {
    d <- elvedata %>%
      filter(Region == r & Aar == a)
...
# sett NA verdier i gjennomsnitt_str til regionale gjennomsnitt
gjennomsnitt_str <- gjennomsnitt_str %>%
  group_by(region) %>%
  mutate_at(
...
for (m in indeks_simuler) {
  ...
  d <- elvedata %>%
    filter(Vdrnr == elveliste$VdrNr[m])
  ...
  # flagg år der det mangler fangst og gjenutsatte og ikke er telling
  ...
  d$Asterisk <- ifelse(
    is.na(d$ExpSmallMed) & is.na(d$FangstAndSmallMed) & is.na(d$Probs_small_med),
    1,
    0
  )
  ...
  # Ulike tellinger gir bare et mål på antall og ikke vekt, vi må derfor estimere vekt basert på gjennomsnitt.
  # For år som har fangst og/eller gjenutsatte regner vi ut gjennomsnittsvekt
  #   fra dette: gjen_vekt+laks_vekt/gjen_ant+Laks_ant
  # For år uten fangst brukes gjennomsnittet av fangst og gjenutsatt i 5 nærmeste år
  # Dersom det er færre enn totalt 5 avliva laks i tidsserien, brukes regional snittvekt
  ...
  if (sum(d$telling) > 0) { # denne delen kjøres bare dersom det har vært telling
    ...
    uten_fangst_smaa <- ifelse(
      d$Obs_laks_ant_u3kg > 0 & d$Laks_vekt_u3kg_sum == 0,
      1,
      0
    )

    # Flagg hvis færre enn 5 laks fanget i år med telling
    lite_fangst_smaa <- ifelse(
      d$Obs_laks_ant_u3kg > 0 & (d$Laks_ant_u3kg_sum < 5 & d$Laks_ant_u3kg_sum > 0),
      1,
      0
    )
    ...
    ### MELLOMLAKS
    # Flagg hvis det mangler fangst i år med telling
    uten_fangst_mellom <- ifelse(
      d$Obs_laks_ant_o3u7kg > 0 & d$Laks_vekt_o3u7kg_sum == 0,
      1,
      0
    )

    # Flagg hvis færre enn 5 laks fanget i år med telling
    lite_fangst_mellom <- ifelse(
      d$Obs_laks_ant_o3u7kg > 0 & (d$Laks_ant_o3u7kg_sum < 5 & d$Laks_ant_o3u7kg_sum > 0),
      1,
      0
    )
    ...
    uten_fangst_stor <- ifelse(
      d$Obs_laks_ant_o7kg > 0 & d$Laks_vekt_o7kg_sum == 0,
      1,
      0
    )

    # Flagg hvis færre enn 5 laks fanget i år med telling
    lite_fangst_stor <- ifelse(
      d$Obs_laks_ant_o7kg > 0 & (d$Laks_ant_o7kg_sum < 5 & d$Laks_ant_o7kg_sum > 0),
      1,
      0
    )
    ...
    d$vekt_mellom_obs <- with(
      d,
      ifelse(
        Obs_laks_ant_o3u7kg > 0,
        Laks_vekt_o3u7kg_sum / Laks_ant_o3u7kg_sum * Obs_laks_ant_o3u7kg,
        NA
      )
    )
    d$vekt_stor_obs <- with(
      d,
      ifelse(
        Obs_laks_ant_o7kg > 0,
  ...
  # lag hjelpefunksjoner for å generere fordelinger
  # funksjonene gjør at vi i større grad kan vektorisere koden og redusere bruk av for-løkker
  lag_triangle_ford <- function(ferdig_matrise, med_verdi, min_verdi, maks_verdi, n_sim) {
    for (i in seq_along(med_verdi)) {
      if (!is.na(med_verdi[i])) {
        ferdig_matrise[i, ] <- rtriangle(
          n = n_sim,
          a = min_verdi[i],
          b = maks_verdi[i],
          c = med_verdi[i]
  ...
  lag_binom_ford <- function(ferdig_matrise, size_vektor, prob_vektor, n_sim) {
    for (i in seq_along(size_vektor)) {
      if (!is.na(size_vektor[i])) {
        ferdig_matrise[i, ] <- rbinom(
          n = n_sim,
          size = round(size_vektor[i]),
          prob = prob_vektor[i] / 100
        )
      }
    }
    ferdig_matrise
  ...
  dist_andel_hunn_u3kg <- lag_triangle_ford(
    empty_matrix,
    d$PrHuSmall,
    d$PrHuSmall - 0.05,
    d$PrHuSmall + 0.05,
  ...
  dist_andel_hunn_37kg <- lag_triangle_ford(
    empty_matrix,
    d$PrHuMellom,
    d$PrHuMellom - 0.05,
    d$PrHuMellom + 0.05,
  ...
  dist_andel_hunn_o7kg <- lag_triangle_ford(
    empty_matrix,
    d$PrHuStor,
    d$PrHuStor - 0.05,
    d$PrHuStor + 0.05,
  ...
  dist_andel_hunn_u3kg_gjen <- lag_triangle_ford(
    empty_matrix,
    d$PrHuSmall_Gjenut,
    d$PrHuSmall_Gjenut - 0.05,
    d$PrHuSmall_Gjenut + 0.05,
    n_sim
  )
  dist_andel_hunn_37kg_gjen <- lag_triangle_ford(
    empty_matrix,
    d$PrHuMellom_Gjenut,
    d$PrHuMellom_Gjenut - 0.05,
    d$PrHuMellom_Gjenut + 0.05,
    n_sim
  )
  dist_andel_hunn_o7kg_gjen <- lag_triangle_ford(
    empty_matrix,
    d$PrHuStor_Gjenut,
    d$PrHuStor_Gjenut - 0.05,
    d$PrHuStor_Gjenut + 0.05,
    n_sim
  )
  ...
  if (sum(d$telling) > 0) {
    ...
    dist_andel_obs_u3kg <- lag_triangle_ford(
      empty_matrix,
      d$Probs_small_med,
      d$Probs_small_min,
      d$Probs_small_max,
    ...
    dist_andel_obs_37kg <- lag_triangle_ford(
      empty_matrix,
      d$Probs_mellom_med,
      d$Probs_mellom_min,
      d$Probs_mellom_max,
    ...
    dist_andel_obs_o7kg <- lag_triangle_ford(
      empty_matrix,
      d$Probs_stor_med,
      d$Probs_stor_min,
      d$Probs_stor_max,
    ...
    oppdrett_dist_totalt <- lag_binom_ford(
      empty_matrix,
      d$Laks_ant,
      d$ProsOpp,
      n_sim
    )
    oppdrett_dist_u3kg <- lag_binom_ford(
      empty_matrix,
      d$Laks_ant_u3kg,
      d$ProsOpp,
      n_sim
    )
    oppdrett_dist_37kg <- lag_binom_ford(
      empty_matrix,
      d$Laks_ant_o3u7kg,
      d$ProsOpp,
      n_sim
    )
    oppdrett_dist_o7kg <- lag_binom_ford(
      empty_matrix,
      d$Laks_ant_o7kg,
      d$ProsOpp,
      n_sim
    ...
    d$ExpSmallMed <- with(
      d,
      ifelse(
        !is.na(Obs_laks_ant_u3kg) & Obs_laks_ant_u3kg > 0 & is.na(ExpSmallMed),
        Laks_ant_u3kg / rowMedians(villaks_antall_u3kg, na.rm = TRUE),
        ExpSmallMed
      )
    )
    d$ExpSmallMed <- with(
    ...
    d$ExpSmallMin <- with(
      d,
      ifelse(
        !is.na(Obs_laks_ant_u3kg) & Obs_laks_ant_u3kg > 0 & is.na(ExpSmallMin),
        Laks_ant_u3kg / as.vector(rowMax(villaks_antall_u3kg)),
        ExpSmallMin
      )
    )
    d$ExpSmallMin <- with(
    ...
    d$ExpSmallMax <- with(
      d,
      ifelse(
        !is.na(Obs_laks_ant_u3kg) & Obs_laks_ant_u3kg > 0 & is.na(ExpSmallMax),
        Laks_ant_u3kg / as.vector(rowMin(villaks_antall_u3kg)),
        ExpSmallMax
      )
    )
    d$ExpSmallMax <- with(
    ...
    d$ExpMellomMed <- with(
      d,
      ifelse(
        !is.na(Obs_laks_ant_o3u7kg) & Obs_laks_ant_o3u7kg > 0 & is.na(ExpMellomMed),
        Laks_ant_o3u7kg / rowMedians(villaks_antall_37kg, na.rm = TRUE),
        ExpMellomMed
      )
    )
    d$ExpMellomMed <- with(
    ...
    d$ExpMellomMin <- with(
      d,
      ifelse(
        !is.na(Obs_laks_ant_o3u7kg) & Obs_laks_ant_o3u7kg > 0 & is.na(ExpMellomMin),
        Laks_ant_o3u7kg / as.vector(rowMax(villaks_antall_37kg)),
        ExpMellomMin
      )
    )
    d$ExpMellomMin <- with(
    ...
    d$ExpMellomMax <- with(
      d,
      ifelse(
        !is.na(Obs_laks_ant_o3u7kg) & Obs_laks_ant_o3u7kg > 0 & is.na(ExpMellomMax),
        Laks_ant_o3u7kg / as.vector(rowMin(villaks_antall_37kg)),
        ExpMellomMax
      )
    )
    d$ExpMellomMax <- with(
    ...
    d$ExpStorMed <- with(
      d,
      ifelse(
        !is.na(Obs_laks_ant_o7kg) & Obs_laks_ant_o7kg > 0 & is.na(ExpStorMed),
        Laks_ant_o7kg / rowMedians(villaks_antall_o7kg,  na.rm = TRUE),
        ExpStorMed
      )
    )
    d$ExpStorMed <- with(
    ...
    d$ExpStorMin <- with(
      d,
      ifelse(
        !is.na(Obs_laks_ant_o7kg) & Obs_laks_ant_o7kg > 0 & is.na(ExpStorMin),
        Laks_ant_o7kg / as.vector(rowMax(villaks_antall_o7kg)),
        ExpStorMin
      )
    )
    d$ExpStorMin <- with(
    ...
    d$ExpStorMax <- with(
      d,
      ifelse(
        !is.na(Obs_laks_ant_o7kg) & Obs_laks_ant_o7kg > 0 & is.na(ExpStorMax),
        Laks_ant_o7kg / as.vector(rowMin(villaks_antall_o7kg)),
        ExpStorMax
      )
    )
    d$ExpStorMax <- with(
  ...
  Beskatning <- function(Laks_ant, Gjen_ant, FangstAnd, Exp) {
    Exp <- ifelse(
      !is.na(Exp) | (is.na(Exp) & is.na(FangstAnd)),
      Exp,
  ...
  d$ExpSmallMed <- with(
    d,
    Beskatning(Laks_ant_u3kg, Gjen_ant_u3kg, FangstAndSmallMed, ExpSmallMed)
  )
  d$ExpSmallMin <- with(
    d,
    Beskatning(Laks_ant_u3kg, Gjen_ant_u3kg, FangstAndSmallMin, ExpSmallMin)
  )
  d$ExpSmallMax <- with(
    d,
    Beskatning(Laks_ant_u3kg, Gjen_ant_u3kg, FangstAndSmallMax, ExpSmallMax)
  )
  d$ExpMellomMed <- with(
    d,
    Beskatning(Laks_ant_o3u7kg, Gjen_ant_o3u7kg, FangstAndMellomMed, ExpMellomMed)
  )
  d$ExpMellomMin <- with(
    d,
    Beskatning(Laks_ant_o3u7kg, Gjen_ant_o3u7kg, FangstAndMellomMin, ExpMellomMin)
  )
  d$ExpMellomMax <- with(
    d,
    Beskatning(Laks_ant_o3u7kg, Gjen_ant_o3u7kg, FangstAndMellomMax, ExpMellomMax)
  )
  d$ExpStorMed <- with(
    d,
    Beskatning(Laks_ant_o7kg, Gjen_ant_o7kg, FangstAndStorMed, ExpStorMed)
  )
  d$ExpStorMin <- with(
    d,
    Beskatning(Laks_ant_o7kg, Gjen_ant_o7kg, FangstAndStorMin, ExpStorMin)
  )
  d$ExpStorMax <- with(
    d,
    Beskatning(Laks_ant_o7kg, Gjen_ant_o7kg, FangstAndStorMax, ExpStorMax)
  )
  ...
  FangstAndel <- function(Laks_ant, Gjen_ant, FangstAnd, Exp) {
    FangstAnd <- ifelse(
      !is.na(FangstAnd) | (is.na(FangstAnd) & (is.na(Exp) | Exp == 0)),
      FangstAnd,
  ...
  d$FangstAndSmallMed <- with(
    d,
    FangstAndel(Laks_ant_u3kg, Gjen_ant_u3kg, FangstAndSmallMed, ExpSmallMed)
  )
  d$FangstAndSmallMed <- with(
    d,
    ifelse(
      is.na(FangstAndSmallMed) & ExpSmallMed == 0,
      0,
      FangstAndSmallMed
    )
  )
  d$FangstAndSmallMin <- with(
    d,
    FangstAndel(Laks_ant_u3kg, Gjen_ant_u3kg, FangstAndSmallMin, ExpSmallMin)
  )
  d$FangstAndSmallMin <- with(
    d,
    ifelse(
  ...
  d$FangstAndSmallMax <- with(
    d,
    FangstAndel(Laks_ant_u3kg, Gjen_ant_u3kg, FangstAndSmallMax, ExpSmallMax)
  )
  d$FangstAndSmallMax <- with(
    d,
    ifelse(
      is.na(FangstAndSmallMax) & ExpSmallMax == 0,
      0,
      FangstAndSmallMax
    )
  )

  d$FangstAndMellomMed <- with(
    d,
    FangstAndel(Laks_ant_o3u7kg, Gjen_ant_o3u7kg, FangstAndMellomMed, ExpMellomMed)
  )
  d$FangstAndMellomMed <- with(
    d,
    ifelse(
      is.na(FangstAndMellomMed) & ExpMellomMed == 0,
      0,
      FangstAndMellomMed
    )
  )
  d$FangstAndMellomMin <- with(
    d,
    FangstAndel(Laks_ant_o3u7kg, Gjen_ant_o3u7kg, FangstAndMellomMin, ExpMellomMin)
  )
  d$FangstAndMellomMin <- with(
    d,
    ifelse(
      is.na(FangstAndMellomMin) & ExpMellomMin == 0,
      0,
      FangstAndMellomMin
    )
  )
  d$FangstAndMellomMax <- with(
    d,
    FangstAndel(Laks_ant_o3u7kg, Gjen_ant_o3u7kg, FangstAndMellomMax, ExpMellomMax)
  )
  d$FangstAndMellomMax <- with(
    d,
    ifelse(
      is.na(FangstAndMellomMax) & ExpMellomMax == 0,
      0,
      FangstAndMellomMax
    )
  )

  d$FangstAndStorMed <- with(
    d,
    FangstAndel(Laks_ant_o7kg, Gjen_ant_o7kg, FangstAndStorMed, ExpStorMed)
  )
  d$FangstAndStorMed <- with(
    d,
    ifelse(
      is.na(FangstAndStorMed) & ExpStorMed == 0,
      0,
      FangstAndStorMed
    )
  )
  d$FangstAndStorMin <- with(
    d,
    FangstAndel(Laks_ant_o7kg, Gjen_ant_o7kg, FangstAndStorMin, ExpStorMin)
  )
  d$FangstAndStorMin <- with(
    d,
    ifelse(
      is.na(FangstAndStorMin) & ExpStorMin == 0,
      0,
      FangstAndStorMin
    )
  )
  d$FangstAndStorMax <- with(
    d,
    FangstAndel(Laks_ant_o7kg, Gjen_ant_o7kg, FangstAndStorMax, ExpStorMax)
  )
  d$FangstAndStorMax <- with(
    d,
    ifelse(
      is.na(FangstAndStorMax) & ExpStorMax == 0,
      0,
      FangstAndStorMax
    )
  )
  ...
  if (paste("Beskatning_FangstAndel", d$Aar[max(n_years)], ".csv", sep = "") %in% list.files("results")) {
    write.table(
      til_fil_beskatning_fangstandel,
      paste(
        "results/Beskatning_FangstAndel", d$Aar[max(n_years)],
        ".csv",
        sep = ""
      ),
      sep = ";",
    ...
    write.table(
      til_fil_beskatning_fangstandel,
      paste(
        "results/Beskatning_FangstAndel",
        d$Aar[max(n_years)],
        ".csv",
        sep = ""
      ),
  ...
  beskatningsrate_u3kg <- lag_triangle_ford(
    empty_matrix,
    d$ExpSmallMed,
    d$ExpSmallMin,
    d$ExpSmallMax,
  ...
  beskatningsrate_37kg <- lag_triangle_ford(
    empty_matrix,
    d$ExpMellomMed,
    d$ExpMellomMin,
    d$ExpMellomMax,
  ...
  beskatningsrate_o7kg <- lag_triangle_ford(
    empty_matrix,
    d$ExpStorMed,
    d$ExpStorMin,
    d$ExpStorMax,
  ...
  fangstandel_u3kg <- lag_triangle_ford(
    empty_matrix,
    d$FangstAndSmallMed,
    d$FangstAndSmallMin,
    d$FangstAndSmallMax,
  ...
  fangstandel_37kg <- lag_triangle_ford(
    empty_matrix,
    d$FangstAndMellomMed,
    d$FangstAndMellomMin,
    d$FangstAndMellomMax,
    n_sim
  )
  fangstandel_o7kg <- lag_triangle_ford(
    empty_matrix,
    d$FangstAndStorMed,
    d$FangstAndStorMin,
    d$FangstAndStorMax,
  ...
  oppdrett_antall_u3kg <- ifelse(
    is.na(beskatningsrate_u3kg) | oppdrett_dist_u3kg == 0 | beskatningsrate_u3kg == 0,
    0,
    oppdrett_dist_u3kg / beskatningsrate_u3kg
  )
  oppdrett_antall_37kg <- ifelse(
    is.na(beskatningsrate_37kg) | oppdrett_dist_37kg == 0 | beskatningsrate_37kg == 0,
    0,
    oppdrett_dist_37kg / beskatningsrate_37kg
  )
  oppdrett_antall_o7kg <- ifelse(
    is.na(beskatningsrate_o7kg) | oppdrett_dist_o7kg == 0 | beskatningsrate_o7kg == 0,
    0,
    oppdrett_dist_o7kg / beskatningsrate_o7kg
  )
  ...
  valgte_rader <- !is.na(fangstandel_u3kg[, 1]) & fangstandel_u3kg[, 1] > 0
  if (any(valgte_rader)) {
    villaks_antall_u3kg[valgte_rader, ] <- (d$Laks_ant_u3kg[valgte_rader] - oppdrett_dist_u3kg[valgte_rader, ] + (d$Gjen_ant_u3kg[valgte_rader] - (d$Gjen_ant_u3kg[valgte_rader] * fangstandel_u3kg[valgte_rader, ] * 0.2))) / fangstandel_u3kg[valgte_rader, ]
  }
  valgte_rader <- !is.na(d$Obs_laks_ant_u3kg) & d$Obs_laks_ant_u3kg > 0
  if (any(valgte_rader)) {
    villaks_antall_u3kg[valgte_rader, ] <- (d$Obs_laks_ant_u3kg[valgte_rader] / dist_andel_obs_u3kg[valgte_rader, ]) + d$Laks_ant_u3kg[valgte_rader] - oppdrett_dist_u3kg[valgte_rader, ]
  }
  # sett eventuelle negative verdier til 0
  villaks_antall_u3kg[villaks_antall_u3kg < 0] <- 0
  # håndter eventuelle NA eller Inf verdier
  villaks_antall_u3kg[is.na(villaks_antall_u3kg) | is.infinite(villaks_antall_u3kg)] <- 0

  valgte_rader <- !is.na(fangstandel_37kg[, 1]) & fangstandel_37kg[, 1] > 0
  if (any(valgte_rader)) {
    villaks_antall_37kg[valgte_rader, ] <- (d$Laks_ant_o3u7kg[valgte_rader] - oppdrett_dist_37kg[valgte_rader, ] + (d$Gjen_ant_o3u7kg[valgte_rader] - (d$Gjen_ant_o3u7kg[valgte_rader] * fangstandel_37kg[valgte_rader, ] * 0.2))) / fangstandel_37kg[valgte_rader, ]
  }
  valgte_rader <- !is.na(d$Obs_laks_ant_o3u7kg) & d$Obs_laks_ant_o3u7kg > 0
  if (any(valgte_rader)) {
    villaks_antall_37kg[valgte_rader, ] <- (d$Obs_laks_ant_o3u7kg[valgte_rader] / dist_andel_obs_37kg[valgte_rader, ]) + d$Laks_ant_o3u7kg[valgte_rader] - oppdrett_dist_37kg[valgte_rader, ]
  }
  # sett eventuelle negative verdier til 0
  villaks_antall_37kg[villaks_antall_37kg < 0] <- 0
  # håndter eventuelle NA eller Inf verdier
  villaks_antall_37kg[is.na(villaks_antall_37kg) | is.infinite(villaks_antall_37kg)] <- 0

  valgte_rader <- !is.na(fangstandel_o7kg[, 1]) & fangstandel_o7kg[, 1] > 0
  if (any(valgte_rader)) {
    villaks_antall_o7kg[valgte_rader, ] <- (d$Laks_ant_o7kg[valgte_rader] - oppdrett_dist_o7kg[valgte_rader, ] + (d$Gjen_ant_o7kg[valgte_rader] - (d$Gjen_ant_o7kg[valgte_rader] * fangstandel_o7kg[valgte_rader, ] * 0.2))) / fangstandel_o7kg[valgte_rader, ]
  }
  valgte_rader <- !is.na(d$Obs_laks_ant_o7kg) & d$Obs_laks_ant_o7kg > 0
  if (any(valgte_rader)) {
    villaks_antall_o7kg[valgte_rader, ] <- (d$Obs_laks_ant_o7kg[valgte_rader] / dist_andel_obs_o7kg[valgte_rader, ]) + d$Laks_ant_o7kg[valgte_rader] - oppdrett_dist_o7kg[valgte_rader, ]
  }
  # sett eventuelle negative verdier til 0
  villaks_antall_o7kg[villaks_antall_o7kg < 0] <- 0
  # håndter eventuelle NA eller Inf verdier
  villaks_antall_o7kg[is.na(villaks_antall_o7kg) | is.infinite(villaks_antall_o7kg)] <- 0

  # Totalt antall villaks
  ...
  til_fil_antall_vill_oppdrett <- tibble(
    villaks_antall_u3kg_q02.5, villaks_antall_u3kg_q25,
  ...
  gjenutsatt_hunn_kg_u3kg_just <- sapply(1:n_sim, function(s) {
    ifelse(
      d$Gjen_vekt_u3kg > 0,
      (d$Gjen_vekt_u3kg - (d$Gjen_vekt_u3kg * fangstandel_u3kg[, s] * 0.2)) * dist_andel_hunn_u3kg[, s],
      0
    )
  })
  gjenutsatt_hunn_kg_37kg_just <- sapply(1:n_sim, function(s) {
    ifelse(
      d$Gjen_vekt_o3u7kg > 0,
      (d$Gjen_vekt_o3u7kg - (d$Gjen_vekt_o3u7kg * fangstandel_37kg[, s] * 0.2)) * dist_andel_hunn_37kg[, s],
      0
    )
  })
  gjenutsatt_hunn_kg_o7kg_just <- sapply(1:n_sim, function(s) {
    ifelse(
      d$Gjen_vekt_o7kg > 0,
      (d$Gjen_vekt_o7kg - (d$Gjen_vekt_o7kg * fangstandel_o7kg[, s] * 0.2)) * dist_andel_hunn_o7kg[, s],
      0
    )
  })
  ...
  avlivet_hunn_kg_u3kg_just <- sapply(1:n_sim, function(s) {
    ifelse(
      d$Laks_vekt_u3kg > 0,
      (d$Laks_vekt_u3kg - oppdrett_kg_u3kg[, s]) * dist_andel_hunn_u3kg[, s],
      0
    )
  })
  avlivet_hunn_kg_37kg_just <- sapply(1:n_sim, function(s) {
    ifelse(
      d$Laks_vekt_o3u7kg > 0,
      (d$Laks_vekt_o3u7kg - oppdrett_kg_37kg[, s]) * dist_andel_hunn_37kg[, s],
      0
    )
  })
  avlivet_hunn_kg_o7kg_just <- sapply(1:n_sim, function(s) {
    ifelse(
      d$Laks_vekt_o7kg > 0,
      (d$Laks_vekt_o7kg - oppdrett_kg_o7kg[, s]) * dist_andel_hunn_o7kg[, s],
      0
    )
  })
  ...
  valgte_rader <- d$Simulering & (is.na(d$Obs_laks_ant_u3kg) | d$Obs_laks_ant_u3kg == 0) & !is.na(fangstandel_u3kg[, 1]) & fangstandel_u3kg[, 1] > 0
  if (any(valgte_rader)) {
    gyting_hunn_kg_u3kg[valgte_rader, ] <- (gjenutsatt_hunn_kg_u3kg_just[valgte_rader, ] + avlivet_hunn_kg_u3kg_just[valgte_rader, ]) / fangstandel_u3kg[valgte_rader, ] - d$StamAntSmaHo[valgte_rader] * 2 - gjenutsatt_hunn_kg_u3kg_just[valgte_rader, ] * andel_dode_gjenuts - avlivet_hunn_kg_u3kg_just[valgte_rader, ]
    gyting_hunn_kg_u3kg[!valgte_rader, ] <- 0
  }
  valgte_rader <- d$Simulering & !is.na(d$Obs_laks_ant_u3kg) & d$Obs_laks_ant_u3kg > 0
  if (any(valgte_rader)) {
    gyting_hunn_kg_u3kg[valgte_rader, ] <- ((d$vekt_smaa_obs[valgte_rader] * dist_andel_hunn_u3kg[valgte_rader, ]) / dist_andel_obs_u3kg[valgte_rader, ] - d$StamAntSmaHo[valgte_rader] * 2)
  }
  # sett eventuelle negative verdier til 0
  gyting_hunn_kg_u3kg[gyting_hunn_kg_u3kg < 0] <- 0
  # håndter eventuelle NA eller Inf verdier
  gyting_hunn_kg_u3kg[is.na(gyting_hunn_kg_u3kg) | is.infinite(gyting_hunn_kg_u3kg)] <- 0

  valgte_rader <- d$Simulering & (is.na(d$Obs_laks_ant_o3u7kg) | d$Obs_laks_ant_o3u7kg == 0) & !is.na(fangstandel_37kg[, 1]) & fangstandel_37kg[, 1] > 0
  if (any(valgte_rader)) {
    gyting_hunn_kg_37kg[valgte_rader, ] <- (gjenutsatt_hunn_kg_37kg_just[valgte_rader, ] + avlivet_hunn_kg_37kg_just[valgte_rader, ]) / fangstandel_37kg[valgte_rader, ] - d$StamAntMelHo[valgte_rader] * 2 - gjenutsatt_hunn_kg_37kg_just[valgte_rader, ] * andel_dode_gjenuts - avlivet_hunn_kg_37kg_just[valgte_rader, ]
    gyting_hunn_kg_37kg[!valgte_rader, ] <- 0
  }
  valgte_rader <- d$Simulering & !is.na(d$Obs_laks_ant_o3u7kg) & d$Obs_laks_ant_o3u7kg > 0
  if (any(valgte_rader)) {
    gyting_hunn_kg_37kg[valgte_rader, ] <- ((d$vekt_mellom_obs[valgte_rader] * dist_andel_hunn_37kg[valgte_rader, ]) / dist_andel_obs_37kg[valgte_rader, ] - d$StamAntMelHo[valgte_rader] * 4)
  }
  # sett eventuelle negative verdier til 0
  gyting_hunn_kg_37kg[gyting_hunn_kg_37kg < 0] <- 0
  # håndter eventuelle NA eller Inf verdier
  gyting_hunn_kg_37kg[is.na(gyting_hunn_kg_37kg) | is.infinite(gyting_hunn_kg_37kg)] <- 0

  valgte_rader <- d$Simulering & (is.na(d$Obs_laks_ant_o7kg) | d$Obs_laks_ant_o7kg == 0) & !is.na(fangstandel_o7kg[, 1]) & fangstandel_o7kg[, 1] > 0
  if (any(valgte_rader)) {
    gyting_hunn_kg_o7kg[valgte_rader, ] <- (gjenutsatt_hunn_kg_o7kg_just[valgte_rader, ] + avlivet_hunn_kg_o7kg_just[valgte_rader, ]) / fangstandel_o7kg[valgte_rader, ] - d$StamAntStorHo[valgte_rader] * 8 - gjenutsatt_hunn_kg_o7kg_just[valgte_rader, ] * andel_dode_gjenuts - avlivet_hunn_kg_o7kg_just[valgte_rader, ]
    gyting_hunn_kg_o7kg[!valgte_rader, ] <- 0
  }
  valgte_rader <- d$Simulering & !is.na(d$Obs_laks_ant_o7kg) & d$Obs_laks_ant_o7kg > 0
  if (any(valgte_rader)) {
    gyting_hunn_kg_o7kg[valgte_rader, ] <- ((d$vekt_stor_obs[valgte_rader] * dist_andel_hunn_o7kg[valgte_rader, ]) / dist_andel_obs_o7kg[valgte_rader, ] - d$StamAntStorHo[valgte_rader] * 8)
  }
  # sett eventuelle negative verdier til 0
  gyting_hunn_kg_o7kg[gyting_hunn_kg_o7kg < 0] <- 0
  # håndter eventuelle NA eller Inf verdier
  gyting_hunn_kg_o7kg[is.na(gyting_hunn_kg_o7kg) | is.infinite(gyting_hunn_kg_o7kg)] <- 0
  ...
  # lag en matrise hvor hver rad inneholder GBM-fordelingsverdiene repeteres for alle år
  gbm_matrix <- matrix(rep(gbm, n_years), nrow = n_years, ncol = n_sim, byrow = TRUE)
  # sammenlign gytebestand med GBM-fordelingen i en vektorisert operasjon
  # og lag en matrise med 1 eller 0 for hver simulering og hvert år
  naadgbm <- !is.na(gyting_hunn_kg_totalt) & (gyting_hunn_kg_totalt > gbm_matrix)
  # konverter matrisen til en 0/1 numerisk matrise
  naadgbm <- naadgbm * 1
  ...
  # lag en matrise som inneholder måloppnåelse for hver simulering og hvert år
  # denne matrisen er samtidig de ikke-trunkerte verdiene av måloppnåelse
  prosent_gbm_utrunk <- gyting_hunn_kg_totalt / rep(gbm, each = n_years) * 100

  # avgrens verdiene oppover til 100 % (= trunkert måloppnåelse)
  prosent_av_gbm_simulert <- pmin(prosent_gbm_utrunk, 100)

  # håndter NA (behold de der gyting_hunn_kg_totalt er NA)
  prosent_gbm_utrunk[is.na(gyting_hunn_kg_totalt)] <- NA
  prosent_av_gbm_simulert[is.na(gyting_hunn_kg_totalt)] <- NA
  ...
  gyting_hunn_kg_totalt_df <- as_tibble(gyting_hunn_kg_totalt) %>%
    setNames(paste0("sim_", 1:n_sim)) %>% # Gi kolonnene navn sim_1, sim_2 osv.
    # legge til år
    mutate(Aar = c(d$Aar)) %>%
    # flytte kolonne Aar fremst i datasettet
    select(Aar, everything()) %>%
    # gjøre om til langt format, sånn at vi får n_sim rader per år
    # og en kolonne med gyting_hunn_kg_totalt
    pivot_longer(
      cols = starts_with("sim_"),
      names_to = "simulert",
      values_to = "gyting_hunn_kg_totalt"
    ) %>%
    mutate(
      simulert = as.numeric(gsub("sim_", "", simulert))
    ) %>%
    left_join(
      select(
        d,
    ...
    # sett NA ved ikke simulert
    mutate(
      gyting_hunn_kg_totalt = if_else(
        Simulering == 0,
        NA_real_,
        gyting_hunn_kg_totalt
      )
    ) %>%
    # fjerne kolonnen Simulering som ikke skal være med i filen
    select(-Simulering)
